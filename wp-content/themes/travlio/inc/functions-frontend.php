...
if ( ! function_exists( 'travlio_category' ) ) {
	function travlio_category( $post ) {
		// format
...
if ( ! function_exists( 'travlio_center_meta' ) ) {
	function travlio_center_meta( $post ) {
		// format
		$post_format = get_post_format();
		...
		if(!is_single()){
			...
			echo "<span class='author'>".esc_html__( ' / By: ', 'travlio' ).'<a href='.esc_url(get_author_posts_url( $id )).'>'.get_the_author().'</a>' .'</span>';
			echo '</div>';
		echo '</div>';
	}
}
...
if ( ! function_exists( 'travlio_full_top_meta' ) ) {
	function travlio_full_top_meta( $post ) {
		// format
		$post_format = get_post_format();
		$header_class = $post_format ? '' : 'border-left';
		...
		if ( ! is_search() ) {
			if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
				...
				comments_popup_link( esc_html__( '0 comments', 'travlio' ), esc_html__( '1 comment' , 'travlio' ), esc_html__( '% comments', 'travlio' ) );
				echo '</span>';
			}
		}
		echo '</header>';
...
if ( ! function_exists( 'travlio_post_tags' ) ) {
	function travlio_post_tags() {
		$posttags = get_the_tags();
		if ( $posttags ) {
			echo '<span class="entry-tags-list"><strong>'.esc_html__( 'Tags: ' , 'travlio' ).'</strong> ';
			$i = 1;
			$size = count( $posttags );
...
if ( ! function_exists( 'travlio_post_format_link_helper' ) ) {
	function travlio_post_format_link_helper( $content = null, $title = null, $post = null ) {
		if ( ! $content ) {
			$post = get_post( $post );
			$title = $post->post_title;
			$content = $post->post_content;
		}
		$link = travlio_get_first_url_from_string( $content );
		if ( ! empty( $link ) ) {
...
if ( !function_exists('travlio_get_page_title') ) {
	function travlio_get_page_title() {
		$title = '';
		if ( !is_front_page() || is_paged() ) {
			...
			if ( is_home() ) {
				$title = esc_html__( 'The Blogs', 'travlio' );
			} elseif (is_category()) {
				$title = esc_html__( 'The Blogs', 'travlio' );
			} elseif (is_day()) {
				...
				if ( get_post_type() == 'to_book' ) {
					$title = esc_html__( 'Tour', 'travlio' );
				...
				$title = esc_html__('Search results for "','travlio')  . get_search_query().'"';
			} elseif ( is_tag() ) {
				$title = esc_html__('Posts tagged "', 'travlio'). single_tag_title('', false) . '"';
			} elseif ( is_author() ) {
				global $author;
...
if ( ! function_exists( 'travlio_breadcrumbs' ) ) {
	function travlio_breadcrumbs() {

		$delimiter = ' ';
		$home = esc_html__('Home', 'travlio');
		...
		if ( !is_front_page() || is_paged()) {
			...
			if (is_category()) {
				...
				$post_type = get_post_type_object(get_post_type());
				if ( get_post_type() == 'to_book' ) {
					echo trim($before) . esc_html__( 'Tour', 'travlio' ) . $after;
				} elseif (is_object($post_type)) {
				...
				echo trim($before) . sprintf(__('Search results for "%s"','travlio'), get_search_query()) . $after;
			} elseif ( is_tag() ) {
				echo trim($before) . sprintf(__('Posts tagged "%s"', 'travlio'), single_tag_title('', false)) . $after;
			} elseif ( is_author() ) {
				global $author;
				$userdata = get_userdata($author);
				echo trim($before) . esc_html__('Articles posted by ', 'travlio') . $userdata->display_name . $after;
			} elseif ( is_home() ) {
				echo trim($before) . esc_html__('The Blogs', 'travlio') . $after;
			}
...
if ( ! function_exists( 'travlio_render_breadcrumbs' ) ) {
	function travlio_render_breadcrumbs() {
		global $post;
		$style = $classes = array();
		...
		if ( is_page() && is_object($post) ) {
			...
			$full_width = apply_filters('travlio_page_content_class', 'container');
		} elseif ( is_singular('post') || is_category() || is_home() || is_search() ) {
			$show = travlio_get_config('show_blog_breadcrumbs', true);
			if ( !$show || is_front_page() ) {
				return ''; 
			}
			$breadcrumb_img = travlio_get_config('blog_breadcrumb_image');
	        $breadcrumb_color = travlio_get_config('blog_breadcrumb_color');
	        $style = array();
	        if ( $breadcrumb_color ) {
	        ...
	        $full_width = apply_filters('travlio_blog_content_class', 'container');
		} elseif ( is_post_type_archive('to_book') || is_singular('to_book') ) {
			$show = travlio_get_config('show_tour_breadcrumbs', true);
			if ( !$show ) {
				return ''; 
			}
			$breadcrumb_img = travlio_get_config('tour_breadcrumb_image');
	        $breadcrumb_color = travlio_get_config('tour_breadcrumb_color');
	        $style = array();
	        if ( $breadcrumb_color ) {
	            $style[] = 'background-color:'.$breadcrumb_color;
	        }
	        ...
	        $full_width = apply_filters('travlio_tour_content_class', 'container');
		}

		$estyle = !empty($style)? ' style="'.implode(";", $style).'"':"";

		$title = travlio_get_page_title();
...
if ( ! function_exists( 'travlio_paging_nav' ) ) {
	function travlio_paging_nav() {
		...
		<nav class="navigation paging-navigation" role="navigation">
			<h1 class="screen-reader-text hidden"><?php esc_html_e( 'Posts navigation', 'travlio' ); ?></h1>
...
if ( ! function_exists( 'travlio_post_nav' ) ) {
	function travlio_post_nav() {
...
if ( !function_exists('travlio_pagination') ) {
    function travlio_pagination($per_page, $total, $max_num_pages = '') {
    	global $wp_query, $wp_rewrite;
        ?>
        <div class="apus-pagination">
        	<?php
        	$prev = '<i class="flaticon-left-arrow-1"></i>';
        	$next = '<i class="flaticon-right-arrow"></i>';
        	$pages = $max_num_pages;
        	$args = array('class'=>'');
...
if ( !function_exists('travlio_comment_form') ) {
	function travlio_comment_form($arg, $class = '') {
		global $post;
		if ('open' == $post->comment_status) {
			ob_start();
	      	...
	      	<div class="commentform reset-button-default">
		    	<div class="clearfix">
			    	<?php
			      	echo str_replace('id="submit"','id="submit" class="btn btn-theme '.$class.'"', $form);
			      	?>
		      	</div>
	      	</div>
	      	<?php
...
if (!function_exists('travlio_list_comment') ) {
	function travlio_list_comment($comment, $args, $depth) {
...
function travlio_comment_field_to_bottom( $fields ) {
	$comment_field = $fields['comment'];
	unset( $fields['comment'] );
	$fields['comment'] = $comment_field;
	return $fields;
}
add_filter( 'comment_form_fields', 'travlio_comment_field_to_bottom' );
 ...
 */
function travlio_create_placeholder($size) {
...
function travlio_display_sidebar_left( $sidebar_configs ) {
	if ( isset($sidebar_configs['left']) ) : ?>
		<div class="<?php echo esc_attr($sidebar_configs['left']['class']) ;?>">
		  	<aside class="sidebar sidebar-left" itemscope="itemscope" itemtype="http://schema.org/WPSideBar">
		  		<div class="close-sidebar-btn hidden-lg hidden-md"> <i class="ti-close"></i> <span><?php esc_html_e('Close', 'travlio'); ?></span></div>
...
function travlio_display_sidebar_right( $sidebar_configs ) {
...
function travlio_before_content( $sidebar_configs ) {
...
add_action('wp_footer', 'travlio_schedule_form_html');
function travlio_schedule_form_html() {
	$schedule_contact_form = travlio_get_config('schedule_contact_form');
	?>
	<div id="apus-schedule-visit-wrapper" class="schedule-visit-form-wrapper mfp-hide">
    	<?php
    	if ( $schedule_contact_form ) {
    		$args = array(
			  'name'        => $schedule_contact_form,
