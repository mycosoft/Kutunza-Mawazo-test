...
//////////////////////////////
    ...
    public static function wp_enqueue_scripts() {
        ...
        if ( BABE_Settings::$settings['google_map_active'] ){
            ///// add google API
            wp_enqueue_script( 'babe-google-api', "https://maps.googleapis.com/maps/api/js?key=".BABE_Settings::$google_api_key."&libraries=places&callback=initDefaultMap", array('jquery', 'babe-js'), BABE_VERSION, true );
        }
...
////////////////////////////
    ...
    public static function babe_post_content_tabs($content, $post_id, $post){
        ...
        if ( !empty(BABE_Settings::$settings['content_in_tabs']) && !empty($tab_titles) ){
            ...
            foreach ( $tab_titles as $ind => $tab_title){
                if ( $ind ){
                    $output .= '<h3 class="babe_post_content_title">'.$tab_title.'</h3>';
                }
    ...
    public static function booking_form($booking_obj_id = 0){
        ...
        $output .= '<form id="booking_form" name="booking_form" method="post" action="" data-post-id="'.$post_id.'" class="booking_form_type_'.$rules_cat['rules']['basic_booking_period'].'">
            ...
            <div class="submit_group">
               ...
               <button class="btn button booking_form_submit" data-post-id="'.$post_id.'"><i class="fas fa-shopping-cart"></i> '.apply_filters('babe_booking_form_submit_button_label', __('Book Now', 'ba-book-everything')).'</button>
            ...
            if (!isset($rules_cat['category_meta']['categories_remove_guests']) || !$rules_cat['category_meta']['categories_remove_guests']){
                ...
                foreach($item['meta']['guests'] as $age_id => $guests_num){
                    ...
                    if ($guests_num){
                        ...
                        $guests_price_html = isset($item['meta']['price_arr']['clear'][$age_id]) ? '
                      ...
                      <tr>
                        ...
                        <td class="order_item_age_guests"><span class="order_item_age_guests_num">x'.$guests_num.'=</span></td>
                      '.$guests_price_html.'
                      </tr>';
                        }
                    }
